<nav x-data="{ open: false }" class="bg-white border-b border-gray-100 print:hidden">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    @foreach ($navlinks['public'] as $navlink)
                        <x-jet-nav-link href="{{ $navlink['path'] }}" :active="$navlink['active']">
                            {{ $navlink['title'] }}
                        </x-jet-nav-link>
                    @endforeach
                    @if(auth()->user() && auth()->user()->isAdmin())
                    @foreach ($navlinks['admin'] as $navlink)
                        <x-jet-nav-link href="{{ $navlink['path'] }}" :active="$navlink['active']">
                            {{ $navlink['title'] }}
                        </x-jet-nav-link>
                    @endforeach
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @foreach ($navlinks['public'] as $navlink)
                <x-jet-responsive-nav-link href="{{ $navlink['path'] }}" :active="$navlink['active']">
                    {{ $navlink['title'] }}
